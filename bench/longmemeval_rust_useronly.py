#!/usr/bin/env python3
"""
LongMemEval benchmark runner for Rust MemPalace — user-turns-only mode.

This is an apples-to-apples comparison with the Python MemPalace 'raw' mode,
which indexes ONLY user turns (not assistant turns). The original
longmemeval_rust.py indexes all turns (user + assistant concatenated).

The only difference from longmemeval_rust.py is in get_haystack_sessions():
  - Here: only content where role == "user" is indexed (no role prefix)
  - Original: all turns are joined as "role: content"

Usage:
  python bench/longmemeval_rust_useronly.py [--data /path/to/longmemeval_s_cleaned.json]
                                             [--limit N]
                                             [--verbose]
"""
...
def get_haystack_sessions_user_only(item: dict) -> list[tuple[str, str]]:
    """
    Returns [(session_id, session_text), ...] indexing ONLY user turns.

    This mirrors Python MemPalace 'raw' mode:
      turns = [t for t in session if t["role"] == "user"]
      text  = " ".join(t["content"] for t in turns)

    (no role prefix, only user content, space-joined)
    """
    sessions = []
    ...
    for i, sess in enumerate(raw_sessions):
        ...
        # Only user turns, space-joined content (matches Python raw mode)
        parts = [t.get("content", "") for t in turns if t.get("role") == "user"]
        text = " ".join(parts)
        if not text.strip():
            # Fallback: if no user turns at all, use everything
...
def run_benchmark(data_path: str, limit: int | None, verbose: bool):
    ...
    print(f"Mode:   USER-TURNS-ONLY (apples-to-apples with Python raw mode)")
    print()
    ...
    for i, item in enumerate(dataset):
        ...
        sessions = get_haystack_sessions_user_only(item)
    ...
    print()
    print("=" * 60)
    print(f"  LongMemEval R@5 — Rust MemPalace (user-turns-only)")
    print("=" * 60)
